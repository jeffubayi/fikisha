...
@section('content')
    <div class="container-fluid">
        ...
        <div class="max-w-2xl mx-auto bg-white rounded-lg  p-16">
            <form action="{{ route('fleets.update') }}" method="POST">
                @csrf
                <div class="mb-6">
                    ...
                    <input type="text" id="description"
                        ...
                        value="{{ $fleet->fleet_name }}" name="fleet_name">
                    @error('description')
                ...
                <div class="grid gap-6 mb-6 lg:grid-cols-2">
                    <div>
                        <label for="fleet_name" class="block mb-2 text-sm font-medium text-gray-900 ">
                            Number Plate</label>
                        <input type="text" name="fleet_plate" id="company" value="{{ $fleet->fleet_plate }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        @error('fleet_name')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select id="countries" name="fleet_status"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                            <option value="{{ $fleet->fleet_status }}" selected>{{ $fleet->fleet_status }}</option>
